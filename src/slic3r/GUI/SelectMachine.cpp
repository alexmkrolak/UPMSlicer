    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour("#BBD800"));
    ...
    m_link_edit_nozzle->SetForegroundColour("#BBD800");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(187, 216, 0));
...
{
    if (enable) {
        m_ams_backup_tip->SetForegroundColour(StateColor::darkModeColorFor("#BBD800"));
    }
    else {
...
void SelectMachineDialog::show_timelapse_storage_dialog(MachineObject* obj)
{
    ...
    if (show_confirm_btn) {
        ...
        StateColor confirm_bg(std::pair<wxColour, int>(wxColour(187, 216, 0), StateColor::Normal));
    ...
    if (!show_confirm_btn) {
        // Orca: use the accent green rather than the reference's hard-coded confirm colour.
        StateColor cancel_bg(std::pair<wxColour, int>(wxColour(187, 216, 0), StateColor::Normal));
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#BBD800") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
