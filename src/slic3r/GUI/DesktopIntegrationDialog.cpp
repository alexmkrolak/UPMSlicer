...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy the Mosso Slicer icon from resources_dir()/images.
        if (contains_path_dir(target_candidates[i], "images")) {
            target_dir_icons = target_candidates[i];
            std::string icon_path = GUI::format("%1%/images/MossoSlicer.png",resources_dir());
            std::string dest_path = GUI::format("%1%/images/%2%MossoSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying Mosso Slicer icon to icons directory failed.";
    } else 
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/images/%2%MossoSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i)
    {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            std::string desktop_file = GUI::format(
                ...
                "StartupNotify=false\n"
                "StartupWMClass=mosso-slicer\n", name_suffix, version_suffix, excutable_path);

            std::string path = GUI::format("%1%/applications/com.upmvisualsolutions.MossoSlicer%2%.desktop", target_dir_desktop, version_suffix);
            if (create_desktop_file(path, desktop_file)){
                BOOST_LOG_TRIVIAL(debug) << "com.upmvisualsolutions.MossoSlicer.desktop file installation success.";
                break;
            } else {
            	// write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to install the Mosso Slicer desktop file failed. failed path: " << target_candidates[i];
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/com.upmvisualsolutions.MossoSlicer%2%.desktop", target_dir_desktop, version_suffix));
